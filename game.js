...
const state = {
  ...
  poisonZones: [],
  rifts: [],
  touchMove: { active: false, origin: null, vec: { x: 0, y: 0 } },
};
...
export function handleTouchStart(e) {
  ...
  for (const touch of e.changedTouches) {
    ...
    if (x < WIDTH * 0.5) {
      state.touchMove = { active: true, origin: { x, y }, vec: { x: 0, y: 0 } };
    } else {
      state.isMouseDown = true;
      state.mousePos = { x, y };
    }
...
export function handleTouchMove(e) {
  ...
  for (const touch of e.changedTouches) {
    const x = ((touch.clientX - rect.left) / rect.width) * WIDTH;
    const y = ((touch.clientY - rect.top) / rect.height) * HEIGHT;
    if (state.touchMove.active && state.touchMove.origin) {
...
export function handleTouchEnd(e) {
  ...
  for (const touch of e.changedTouches) {
    ...
    if (state.touchMove.active) {
      state.touchMove = { active: false, origin: null, vec: { x: 0, y: 0 } };
    }
    if (x) {
      state.isMouseDown = false;
    }
