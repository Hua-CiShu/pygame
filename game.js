...
const state = {
  ...
  bossDefeated: 0,
  inputLocked: false,
  ultimateIncoming: false,
  ultimateActive: false,
  ultimateTimer: 0,
  ultimateScale: 1,
  ultimateCamOffsetY: 0,
  ultimateRift: null,
  ultimateBoss: null,
  ultimateCracks: [],
  ultimateHazards: [],
  ultimateDefeatedOnce: false,
};
...
export function initGame(canvasEl) {
  ...
  initStarfield();
  initAudio();
  // Debug helper: allow manual summon of ultimate boss via console
  window.nhwSummonUltimate = () => {
    if (!state.ultimateIncoming && !state.ultimateActive) beginUltimateSummon();
  };
  resetGame();
}
...
function resetCommonState() {
  ...
  state.ultimateIncoming = false;
  state.ultimateActive = false;
  state.ultimateTimer = 0;
  state.ultimateScale = 1;
  state.ultimateCamOffsetY = 0;
  state.ultimateRift = null;
  state.ultimateBoss = null;
  state.ultimateCracks = [];
  state.ultimateHazards = [];
  state.ultimateDefeatedOnce = false;
}
...
function initStarfield() {
  ...
  state.backgroundBlobs = Array.from({ length: 6 }, () => ({
    x: Math.random() * WIDTH,
    y: Math.random() * HEIGHT,
    r: randRange(160, 260),
    color: Math.random() > 0.5 ? "rgba(120,80,255,0.25)" : "rgba(40,200,255,0.22)",
  }));
}
...
function maybeTriggerUltimate() {
  ...
  if (state.ultimateDefeatedOnce) return;
...
function beginUltimateSummon() {
  state.ultimateIncoming = true;
  state.ultimateTimer = 0;
  ...
  state.ultimateScale = 1;
  state.ultimateCamOffsetY = 0;
  state.ultimateRift = { progress: 0, open: false, x: state.width / 2, y: state.height * 0.3 };
  state.ultimateCracks = Array.from({ length: 9 }, () => ({
    x: randRange(80, state.width - 80),
    y: randRange(state.height * 0.55, state.height * 0.9),
    len: randRange(60, 140),
    rot: randRange(-25, 25) * (Math.PI / 180),
  }));
  state.ultimateHazards = [];
  state.effectBanner = { text: "终极Boss逼近…", color: "#FF4D88", timer: 240 };
  state.bossActive = true; // block普通Boss刷新
}

function updateUltimateSummon() {
  const tPrev = state.ultimateTimer;
  state.ultimateTimer += delta;
  const t = state.ultimateTimer;
  const prelude = 420; // ~7s
  const quake = 300; // ~5s
  const emerge = 240; // ~4s
  const calmLead = 90; // 0.5s before裂缝显形不再震动
  // Prelude: ramping shake up to max at 7s
  if (t < prelude) {
    const p = Math.min(1, t / prelude);
    state.shakeTimer = Math.max(state.shakeTimer, 8 + p * 12);
    state.shakeMag = Math.max(state.shakeMag, 6 + p * 10);
  }
  // 0-prelude: 预兆锁定
  if (t >= prelude) {
    const local = t - prelude;
    const phase = Math.min(1, local / quake);
    // hold max shake during quake window
    if (local < calmLead) {
      const maxShake = { timer: 26, mag: 18 };
      state.shakeTimer = Math.max(state.shakeTimer, maxShake.timer);
      state.shakeMag = Math.max(state.shakeMag, maxShake.mag);
    } else {
      // calm window: no shake so裂缝/出场清晰
      state.shakeTimer = 0;
      state.shakeMag = 0;
    }
    const targetScale = 0.75;
    state.ultimateScale = 1 - (1 - targetScale) * phase;
    state.ultimateCamOffsetY = 60 * phase;
    // jitter player for地震感
    if (local < calmLead) {
      state.player.x += randRange(-0.8, 0.8) * delta;
      state.player.y += randRange(-0.8, 0.8) * delta;
    }
    if (local >= 120 && tPrev < prelude + 120) {
      // 粉碎小怪与子弹
    ...
    if (local >= 120) {
      const rProgress = Math.min(1, (local - 120) / (quake - 120));
      state.ultimateRift = {
        progress: rProgress,
        open: true,
        x: state.width / 2,
    ...
    // 将玩家推向下方
    const targetY = state.height * 0.82;
    state.player.y += (targetY - state.player.y) * 0.04 * delta;
  }
  if (t >= prelude + quake + emerge) {
    spawnUltimateBoss();
  }
}

function spawnUltimateBoss() {
  let hp;
  if (state.mode === "rogue") {
    const dmg = Math.max(1, state.playerDamage ?? 1);
    const tScale = Math.max(0, (state.modeElapsed ?? 0) / 600); // 每10分钟增加一档
    const base = 3200 + 220 * Math.pow(dmg, 1.15) + 400 * tScale;
    hp = base * (state.diffHpMult ?? 1);
  } else {
    const lvl = Math.max(1, state.level ?? 1);
    const tScale = Math.max(0, (state.modeElapsed ?? 0) / 600);
    hp = 1800 + 90 * lvl + 300 * tScale;
  }
  const boss = {
    name: "ultimate",
    behavior: "ultimate",
    ...
    baseSpeed: 0.95,
    radius: 85,
    pos: { x: state.width / 2, y: state.height * 0.35 },
    hp,
    maxHp: hp,
    preferDist: 260,
  };
  state.enemies.push(boss);
  state.ultimateBoss = boss;
  state.ultimateIncoming = false;
  ...
  state.shakeTimer = 24;
  state.shakeMag = 8;
  state.ultimateScale = 0.8;
  state.ultimateCamOffsetY = 60;
  state.ultimateRift = { progress: 1, open: true, x: state.width / 2, y: state.height * 0.32 };
  state.effectBanner = { text: "终极Boss降临", color: "#C81CFF", timer: 240 };
...
function moveEnemies() {
  ...
  if (state.ultimateIncoming) {
    for (const enemy of state.enemies) {
      enemy.pos.x += Math.sin(performance.now() * 0.004 + enemy.pos.x) * 0.3;
      enemy.pos.y += Math.cos(performance.now() * 0.004 + enemy.pos.y) * 0.3;
    }
    return;
  }
  for (const enemy of state.enemies) {
    ...
    switch (enemy.behavior) {
      ...
      case "ultimate": {
        ...
        enemy.orbitAngle += 0.004 * enemy.orbitDir * delta;
        const target = {
          x: state.player.x + Math.cos(enemy.orbitAngle) * desired,
          y: state.player.y + Math.sin(enemy.orbitAngle) * desired,
        };
        enemy.pos.x += (target.x - enemy.pos.x) * 0.025 * delta;
        enemy.pos.y += (target.y - enemy.pos.y) * 0.025 * delta;
        break;
      }
    }
  }
...
function updateBossBehavior(enemy) {
  ...
  } else if (enemy.isUltimate) {
    // handled separately in updateUltimateBoss
  } else {
...
function updateUltimateBoss(enemy) {
  if (!enemy || enemy.hp <= 0) return;
  enemy.ultTimer = (enemy.ultTimer ?? 0) + delta;
  const hpRatio = Math.max(0, (enemy.hp ?? 1) / (enemy.maxHp || enemy.hp || 1));
  let phase = 1;
  if (hpRatio <= 0.7 && hpRatio > 0.3) phase = 2;
  else if (hpRatio <= 0.3) phase = 3;

  // gentle float handled in moveEnemies; here only attacks
  const now = enemy.ultTimer;
  const cd = (key, base) => {
    enemy.ultCd = enemy.ultCd || {};
    enemy.ultCd[key] = (enemy.ultCd[key] ?? 0) - delta;
    if (enemy.ultCd[key] <= 0) {
      enemy.ultCd[key] = base;
      return true;
    }
    return false;
  };
  ...
  // Phase attacks
  if (phase === 1) {
    if (cd("fan", 120)) fireUltimateFan(enemy, 9, 0.5, ENEMY_BULLET_SPEED * 1.2);
    if (cd("ring", 210)) fireUltimateRing(enemy, 18, ENEMY_BULLET_SPEED);
    if (cd("hazard", 240)) spawnUltimateHazard(enemy, 120, 18);
  } else if (phase === 2) {
    if (cd("fan", 90)) fireUltimateFan(enemy, 11, 0.65, ENEMY_BULLET_SPEED * 1.35);
    if (cd("spiral", 75)) fireUltimateSpiral(enemy, 18, ENEMY_BULLET_SPEED * 1.1);
    if (cd("hazard", 180)) spawnUltimateHazard(enemy, 140, 22);
    if (cd("pull", 260)) pullPlayerToCenter();
  } else {
    if (cd("fan", 70)) fireUltimateFan(enemy, 13, 0.8, ENEMY_BULLET_SPEED * 1.45);
    if (cd("spiral", 60)) fireUltimateSpiral(enemy, 22, ENEMY_BULLET_SPEED * 1.2);
    if (cd("cross", 130)) shootWardenCross(enemy, 12, ENEMY_BULLET_SPEED * 1.25);
    if (cd("hazard", 150)) spawnUltimateHazard(enemy, 160, 26);
    if (cd("pull", 200)) pullPlayerToCenter();
  }
}

function fireUltimateFan(enemy, count, spread, speed) {
...
function fireUltimateRing(enemy, count, speed) {
  const offset = enemy.ultRingOffset ?? 0;
  for (let i = 0; i < count; i += 1) {
  ...
  enemy.ultRingOffset = offset + 0.18;
}

function fireUltimateSpiral(enemy, count, speed) {
  const offset = enemy.ultSpiralOffset ?? 0;
  for (let i = 0; i < count; i += 1) {
    const ang = (Math.PI * 2 * i) / count + offset;
    state.enemyBullets.push({
      pos: { x: enemy.pos.x, y: enemy.pos.y },
  ...
  enemy.ultSpiralOffset = offset + 0.25;
}

function spawnUltimateHazard(enemy, delay = 140, radius = 20) {
  const hx = randRange(80, state.width - 80);
  const hy = randRange(120, state.height - 80);
  state.ultimateHazards.push({
    x: hx,
    y: hy,
    radius,
    telegraph: delay,
...
function updateUltimateHazards() {
  state.ultimateHazards = (state.ultimateHazards || []).filter((h) => {
    if (!h.active) {
      h.telegraph -= delta;
      if (h.telegraph <= 0) {
        h.active = true;
    ...
    return h.active ? h.duration > 0 : true;
  });
}

function pullPlayerToCenter() {
  const target = { x: state.width / 2, y: state.height * 0.6 };
  state.player.x += (target.x - state.player.x) * 0.08 * delta;
  state.player.y += (target.y - state.player.y) * 0.08 * delta;
}
...
function checkCollisions() {
  ...
  if (state.ultimateHazards?.length) {
    for (const hz of state.ultimateHazards) {
      if (!hz.active) continue;
      const d = Math.hypot(state.player.x - hz.x, state.player.y - hz.y);
      if (d <= hz.radius) {
        handlePlayerDamage();
        break;
      }
...
function updateGame() {
  ...
  maybeTriggerUltimate();
  if (state.ultimateIncoming) {
    updateUltimateSummon();
    updateUltimateHazards();
    return;
  }
  if (!state.gameOver) {
    const mod = getModeModule();
    ...
    movePlayer();
    moveEnemies();
    if (state.ultimateActive && state.ultimateBoss) updateUltimateBoss(state.ultimateBoss);
    updateUltimateHazards();
    updateOrbitals();
    updateBullets();
    checkCollisions();
...
function drawBackground() {
  ...
  gradient.addColorStop(0, "#0c0f2c");
  gradient.addColorStop(0.35, "#080b1f");
  gradient.addColorStop(1, "#04050f");
  ctx.fillStyle = gradient;
  ctx.fillRect(0, 0, WIDTH, HEIGHT);

  // Nebula blobs
  if (state.backgroundBlobs?.length) {
    ctx.save();
    for (const blob of state.backgroundBlobs) {
  ...
  ctx.fillStyle = "rgba(120,255,255,0.3)";
  for (let i = 0; i < 3; i += 1) {
    const x = (WIDTH / 3) * i + 120;
    ctx.beginPath();
    ctx.moveTo(x, 0);
...
function drawUltimateRift() {
  const rift = state.ultimateRift;
  if (!rift?.open) return;
  const p = rift.progress ?? 0;
  const radius = 50 + 70 * p;
  const x = rift.x ?? WIDTH / 2;
  const y = rift.y ?? HEIGHT * 0.3;
  ...
  for (let i = 0; i < spikes; i += 1) {
    ...
    const r = radius * (0.6 + 0.25 * Math.sin(t + i * 1.7));
    pts.push({ x: x + Math.cos(ang) * r, y: y + Math.sin(ang) * r });
  }
  ctx.fillStyle = "rgba(8,0,20,0.92)";
  ctx.beginPath();
  ...
  g.addColorStop(0, "rgba(90,0,140,0.45)");
  g.addColorStop(1, "rgba(0,0,0,0)");
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.moveTo(pts[0].x, pts[0].y);
...
function drawUltimateCracks() {
  if (!state.ultimateCracks?.length) return;
  ctx.save();
  ctx.strokeStyle = "rgba(255,255,255,0.15)";
  ctx.lineWidth = 2;
  ctx.setLineDash([8, 6]);
  for (const c of state.ultimateCracks) {
    const len = c.len ?? 80;
    const x2 = c.x + Math.cos(c.rot) * len;
    const y2 = c.y + Math.sin(c.rot) * len;
    ctx.beginPath();
    ctx.moveTo(c.x, c.y);
    ctx.lineTo(x2, y2);
    ctx.stroke();
...
function drawUltimateHazards() {
  if (!state.ultimateHazards?.length) return;
  ctx.save();
  for (const hz of state.ultimateHazards) {
    const alpha = hz.active ? 0.45 : 0.3 + 0.2 * Math.sin(performance.now() * 0.01);
    ctx.strokeStyle = hz.color ?? "#FF4D88";
    ctx.fillStyle = hz.active ? "rgba(255,77,136,0.18)" : "rgba(255,77,136,0.08)";
    ctx.lineWidth = hz.active ? 3 : 2;
    ctx.beginPath();
...
function drawEnemyShape(enemy) {
  ...
  switch (enemy.behavior) {
    case "slow":
      // layered diamond + inner core
      ctx.fillStyle = gradient;
      const slowR = enemy.radius;
      ctx.beginPath();
      ctx.moveTo(enemy.pos.x, enemy.pos.y - slowR);
      ctx.lineTo(enemy.pos.x + slowR, enemy.pos.y);
      ctx.lineTo(enemy.pos.x, enemy.pos.y + slowR);
      ctx.lineTo(enemy.pos.x - slowR, enemy.pos.y);
      ctx.closePath();
      ctx.fill(); ctx.stroke();
      ctx.save();
      ctx.globalAlpha = 0.8;
      ctx.beginPath();
      ctx.arc(enemy.pos.x, enemy.pos.y, slowR * 0.4, 0, Math.PI * 2);
    ...
    case "zigzag": {
      const pts = [
        ...
        { x: enemy.pos.x + enemy.radius * 0.9, y: enemy.pos.y },
        { x: enemy.pos.x, y: enemy.pos.y + enemy.radius },
        { x: enemy.pos.x - enemy.radius * 0.9, y: enemy.pos.y },
      ];
      ...
      ctx.beginPath();
      ctx.moveTo(enemy.pos.x - enemy.radius * 0.7, enemy.pos.y);
      ctx.lineTo(enemy.pos.x + enemy.radius * 0.7, enemy.pos.y);
      ctx.stroke();
      break;
    }
    case "shooter": {
      ...
      ctx.beginPath(); ctx.moveTo(pts[0].x, pts[0].y); for (let i = 1; i < pts.length; i += 1) ctx.lineTo(pts[i].x, pts[i].y);
      ctx.closePath(); ctx.fill(); ctx.strokeStyle = COLORS.cyan; ctx.stroke();
      ctx.beginPath(); ctx.arc(enemy.pos.x, enemy.pos.y, enemy.radius * 0.45, 0, Math.PI * 2); ctx.stroke();
      // barrel
      ctx.beginPath();
      ctx.moveTo(enemy.pos.x, enemy.pos.y - enemy.radius * 0.2);
      ctx.lineTo(enemy.pos.x, enemy.pos.y - enemy.radius * 1.2);
      ctx.stroke();
      break;
    }
    case "splitter":
      ...
      ctx.beginPath(); ctx.arc(enemy.pos.x, enemy.pos.y, enemy.radius * 1.2, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
      ctx.beginPath(); ctx.moveTo(enemy.pos.x - enemy.radius * 0.6, enemy.pos.y); ctx.lineTo(enemy.pos.x + enemy.radius * 0.6, enemy.pos.y); ctx.stroke();
      break;
    case "brute": {
      ...
      ctx.beginPath(); ctx.moveTo(enemy.pos.x - enemy.radius, enemy.pos.y); ctx.lineTo(enemy.pos.x + enemy.radius, enemy.pos.y); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(enemy.pos.x, enemy.pos.y - enemy.radius); ctx.lineTo(enemy.pos.x, enemy.pos.y + enemy.radius); ctx.stroke();
      break;
    }
    case "commander":
      ctx.beginPath(); ctx.arc(enemy.pos.x, enemy.pos.y, enemy.radius, 0, Math.PI * 2); ctx.fill(); ctx.stroke();
    ...
    case "assassin":
      ...
      ctx.lineTo(enemy.pos.x + enemy.radius * 0.8, enemy.pos.y);
      ctx.lineTo(enemy.pos.x, enemy.pos.y + enemy.radius);
      ctx.lineTo(enemy.pos.x - enemy.radius * 0.8, enemy.pos.y);
      ctx.closePath(); ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(enemy.pos.x - enemy.radius * 0.5, enemy.pos.y);
      ctx.lineTo(enemy.pos.x, enemy.pos.y - enemy.radius * 0.4);
      ctx.lineTo(enemy.pos.x + enemy.radius * 0.5, enemy.pos.y);
      ctx.stroke();
    ...
    case "rift":
      ...
      ctx.lineWidth = 2;
      const ring = regularPolygonPoints(enemy.pos.x, enemy.pos.y, enemy.radius, 12, performance.now() * 0.02);
      ctx.beginPath(); ctx.moveTo(ring[0].x, ring[0].y); for (let i = 1; i < ring.length; i += 1) ctx.lineTo(ring[i].x, ring[i].y);
      ctx.closePath(); ctx.stroke();
      ctx.beginPath();
      ctx.arc(enemy.pos.x, enemy.pos.y, enemy.radius * 0.5, 0, Math.PI * 2);
      ctx.stroke();
      ctx.restore(); break;
    case "charger": {
      const s = enemy.radius * 2;
    ...
    case "ultimate": {
      ...
      const pulse = 1 + 0.08 * Math.sin(performance.now() * 0.0035);
      ctx.shadowColor = enemy.color;
      ctx.shadowBlur = 40;
      // Aura
      const aura = ctx.createRadialGradient(enemy.pos.x, enemy.pos.y, enemy.radius * 0.5, enemy.pos.x, enemy.pos.y, enemy.radius * 1.8);
      aura.addColorStop(0, "rgba(40,0,70,0.7)");
      aura.addColorStop(1, "rgba(0,0,0,0)");
      ctx.fillStyle = aura;
      ctx.beginPath(); ctx.arc(enemy.pos.x, enemy.pos.y, enemy.radius * 1.6 * pulse, 0, Math.PI * 2); ctx.fill();
      // Outer fractured ring + bone spikes
      ctx.strokeStyle = "rgba(180,40,255,0.85)";
      ctx.lineWidth = 6;
      ctx.beginPath(); ctx.arc(enemy.pos.x, enemy.pos.y, enemy.radius * 1.5 * pulse, 0, Math.PI * 2); ctx.stroke();
      ctx.strokeStyle = "rgba(255,100,200,0.7)";
      ctx.lineWidth = 3;
      for (let i = 0; i < 12; i += 1) {
        const ang = (Math.PI * 2 * i) / 12 + t * 0.4;
        const r1 = enemy.radius * 1.2;
        const r2 = enemy.radius * 1.6;
        ctx.beginPath();
      ...
      // Core hull (low-poly body)
      const hull = regularPolygonPoints(enemy.pos.x, enemy.pos.y, enemy.radius * 0.95, 12, (t * 40) % 360);
      const g = ctx.createRadialGradient(enemy.pos.x, enemy.pos.y, enemy.radius * 0.15, enemy.pos.x, enemy.pos.y, enemy.radius);
      g.addColorStop(0, "rgba(20,0,40,0.95)");
      g.addColorStop(1, "rgba(80,20,140,0.85)");
      ctx.fillStyle = g;
      ctx.beginPath(); ctx.moveTo(hull[0].x, hull[0].y); for (let i = 1; i < hull.length; i += 1) ctx.lineTo(hull[i].x, hull[i].y);
      ctx.closePath(); ctx.fill(); ctx.strokeStyle = enemy.color; ctx.lineWidth = 3.5; ctx.stroke();
      // Inner core + eye slit
      const core = ctx.createRadialGradient(enemy.pos.x, enemy.pos.y, 0, enemy.pos.x, enemy.pos.y, enemy.radius * 0.5);
      core.addColorStop(0, "rgba(255,90,200,0.9)");
      core.addColorStop(1, "rgba(60,0,90,0.6)");
      ctx.fillStyle = core;
      ctx.beginPath(); ctx.arc(enemy.pos.x, enemy.pos.y, enemy.radius * 0.5, 0, Math.PI * 2); ctx.fill();
      ctx.strokeStyle = "rgba(255,255,255,0.8)";
      ctx.lineWidth = 4;
      ctx.beginPath();
      ctx.moveTo(enemy.pos.x - enemy.radius * 0.35, enemy.pos.y);
      ctx.quadraticCurveTo(enemy.pos.x, enemy.pos.y - enemy.radius * 0.15, enemy.pos.x + enemy.radius * 0.35, enemy.pos.y);
      ctx.stroke();
      // Crack lines + glow veins
      ctx.strokeStyle = "rgba(255,140,220,0.6)";
      ctx.lineWidth = 2.6;
      for (let i = 0; i < 9; i += 1) {
        const ang = (Math.PI * 2 * i) / 9 + t * 0.6;
        const r1 = enemy.radius * 0.2;
        const r2 = enemy.radius * (0.7 + 0.2 * Math.sin(t + i));
        ctx.beginPath();
        ctx.moveTo(enemy.pos.x + Math.cos(ang) * r1, enemy.pos.y + Math.sin(ang) * r1);
        ctx.lineTo(enemy.pos.x + Math.cos(ang) * r2, enemy.pos.y + Math.sin(ang) * r2);
        ctx.stroke();
      }
      // Tentacles / grasping arms (thick, segmented with claws)
      const arms = 7;
      for (let i = 0; i < arms; i += 1) {
        const ang = (Math.PI * 2 * i) / arms + Math.sin(t * 1.1) * 0.15;
        const base = enemy.radius * 0.9;
        const mid = enemy.radius * 1.4 + 10 * Math.sin(t * 0.9 + i);
        const tip = enemy.radius * 2.0 + 18 * Math.sin(t * 0.6 + i);
        const start = { x: enemy.pos.x + Math.cos(ang) * base, y: enemy.pos.y + Math.sin(ang) * base };
        const midPt = { x: enemy.pos.x + Math.cos(ang) * mid + Math.sin(t + i) * 14, y: enemy.pos.y + Math.sin(ang) * mid + Math.cos(t + i) * 14 };
        const end = { x: enemy.pos.x + Math.cos(ang) * tip, y: enemy.pos.y + Math.sin(ang) * tip };
        ctx.save();
        ...
        ctx.quadraticCurveTo(midPt.x - 12, midPt.y - 8, start.x, start.y);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
        // Claw tip with spikes
        ctx.fillStyle = "rgba(255,120,220,0.8)";
        ctx.beginPath();
        ctx.moveTo(end.x, end.y - 10);
        ctx.lineTo(end.x + 12, end.y + 12);
        ctx.lineTo(end.x - 12, end.y + 12);
        ctx.closePath();
        ctx.fill();
        ctx.stroke();
...
function drawUltimateBar() {
  const boss = state.ultimateBoss;
  if (!state.ultimateActive || !boss || boss.hp <= 0) return;
  const w = 520;
  const h = 18;
  const barX = WIDTH / 2 - w / 2;
  const barY = 12;
  ...
  ctx.fillRect(barX, barY, w, h);
  ctx.fillStyle = "#C81CFF";
  ctx.fillRect(barX + 4, barY + 4, (w - 8) * pct, h - 8);
  ctx.strokeStyle = COLORS.white;
  ctx.lineWidth = 2;
  ctx.strokeRect(barX, barY, w, h);
  ctx.fillStyle = COLORS.white;
  ctx.font = "16px Segoe UI";
  ctx.textAlign = "center";
  ctx.fillText("终极Boss", barX + w / 2, barY + 14);
  ctx.textAlign = "start";
  ctx.restore();
}
...
function renderGame() {
  ctx.save();
  // Background should not shrink; draw before world zoom
  drawBackground();
  // World layer with zoom
  ctx.save();
  ...
  if (state.ultimateScale !== 1 || state.ultimateCamOffsetY !== 0) {
    ctx.translate(WIDTH / 2, HEIGHT / 2 + state.ultimateCamOffsetY);
    ctx.scale(state.ultimateScale, state.ultimateScale);
